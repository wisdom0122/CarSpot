...
const baseURL = Constants.apiUrl + Key + Constants.spotArg;

const getCloseSpot = (current_position) => {
  SpotList.map(
    (spot) =>
      (spot.distance =
        (current_position.lng - spot.lng) ** 2 +
        (current_position.lat - spot.lat) ** 2)
  );
        ...
        res
          .map(
            (r) =>
              xmlToJson(
                parser
                  .parseFromString(r.data, "text/xml")
                  .getElementsByTagName("PRK_STTS")[0]
              ).PRK_STTS
          )
          .map((e) => {
